                ...
                if (result.Succeeded)
                {
                    ...
                    foreach (var roleName in new[] { "User", "Admin" })
                    {
                        if (!await _roleManager.RoleExistsAsync(roleName))
                        {
                            ...
                            {
                                _logger.LogError($"Erreur lors de la création du rôle '{roleName}'.");
                                return Page();
                            }
                        }
                    }

                    // Ajout de l'utilisateur aux rôles
                    if (!await _userManager.IsInRoleAsync(user, "User"))
                    {
                        ...
                        if (!addRoleResult.Succeeded)
                        {
                            ...
                            {
                                _logger.LogError($"Erreur lors de l'ajout du rôle 'User' : {error.Description}");
                            }
                            ModelState.AddModelError(string.Empty, "Erreur lors de l'ajout du rôle.");
                            return Page();
                        }
                    }
